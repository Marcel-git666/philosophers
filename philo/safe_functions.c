...
static void	handle_mutex_error(int status)
{
	...
	if (status == EINVAL)
		...
		error_message("A deadlock would occur if the"
			" thread blocked waiting for mutex.");
	else if (status == EPERM)
		error_message("The current thread doesn't hold a lock on mutex.");
	else if (status == ENOMEM)
		error_message("The process cannot allocate enough"
